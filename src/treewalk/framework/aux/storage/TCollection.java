...
package treewalk.framework.aux.storage;
...
public class TCollection<T>
{
    ...
    public TArray<String> GetKeys ()
    {
        ...
        ret = new TArray<String> ();
        if (fValues.length >= 1)
        {
            for (i = 0; i < fValues.length; i++)
            {
                node = fValues [i];
                k    = (String) node.$get ("key");
                ret.Push (k);
            }
        }
    ...
    public boolean HasElement (String key)
    {
        ...
        ret = fHashMap.hasOwnProperty (key);
